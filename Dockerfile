...
RUN apt-get update && apt-get install -y curl wget ca-certificates \
 && echo "deb http://apt.postgresql.org/pub/repos/apt/ xenial-pgdg  main ${PG_VER}" > /etc/apt/sources.list.d/pgdg.list && wget --quiet -O - https://www.postgresql.org/media/keys/ACCC4CF8.asc | apt-key add - \
 && apt-get update \
 && apt-get install -y autoconf build-essential cmake docbook-mathml docbook-xsl libboost-dev libboost-thread-dev libboost-filesystem-dev libboost-system-dev libboost-iostreams-dev libboost-program-options-dev libboost-timer-dev libcunit1-dev libgdal-dev libgeos++-dev libgeotiff-dev libgmp-dev libjson0-dev libjson-c-dev liblas-dev libmpfr-dev libopenscenegraph-dev libpq-dev libproj-dev libxml2-dev postgresql-server-dev-${PG_VER} xsltproc git build-essential wget libcgal-dev postgresql-common postgresql-contrib-${PG_VER} postgresql-${PG_VER}-plr \
 ...
 && cd postgis-${POSTGIS_VER} && ./configure --with-sfcgal=/usr/local/bin/sfcgal-config --with-geosconfig=/usr/local/bin/geos-config \
 && make && make install \
...
#	RUN echo "host all  all    0.0.0.0/0  md5" >> /etc/postgresql/${PG_VER}/main/pg_hba.conf

#	# And add ``listen_addresses`` to ``/etc/postgresql/${PG_VER}/main/postgresql.conf``
